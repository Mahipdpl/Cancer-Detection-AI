...
def trainAdam():
    ...
    adam_cnn.add(Dense(units = 256, activation = 'relu'))
    adam_cnn.add(Dense(units = y_train.shape[1], activation = 'softmax'))
...
def trainSGD():
    ...
    sgd_cnn.add(MaxPooling2D(pool_size = (2, 2)))
    sgd_cnn.add(Flatten())
    sgd_cnn.add(Dense(units = 256, activation = 'relu'))
    sgd_cnn.add(Dense(units = y_train.shape[1], activation = 'softmax'))
    opt = SGD(lr = 0.001)
...
def trainBatch():
    ...
    mini_cnn.add(Dense(units = 256, activation = 'relu'))
    mini_cnn.add(Dense(units = y_train.shape[1], activation = 'softmax'))
    opt = SGD(lr = 0.001)
